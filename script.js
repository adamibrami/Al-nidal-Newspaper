...
// 2. جلب كافة الأعداد من قاعدة البيانات
async function fetchIssues() {
    ...
    if (error) {
        ...
        document.getElementById('main-h').innerText = "فشل الاتصال بالأرشيف.";
    } else {
        nidalArchive = data;
        renderDropdown();
        if (data && data.length > 0) {
            loadOtherIssue(data.length - 1); // عرض أحدث عدد
        } else {
